...
def scaledDepthMap(depth, color='terrain', saveas = False):
    ...
    fig, ax = plt.subplots()
    h = ax.imshow(depth, cmap = color, vmin=-200, vmax=900)
    fig.colorbar(h)
...
def DEPRECATED_segmentedORBDetect(img, numFeatures, nBlocks=(2,2)):
    numRows, numCols = nBlocks
    horizontal = np.array_split(img, numRows)
    splitted_img = [np.array_split(block, numCols, axis=1) for block in horizontal]
    ...
    for row in range(numRows):
        for col in range(numCols):
            ...
            for kp in keypoints:
                kp.pt = (kp.pt[0]+widthOffset, kp.pt[1]+heightOffset)
                result.append(kp)
    assert len(result) == numFeatures, "Function segmentedORBDetect: Missmatch between numFeatures and result size."
    return result

def segmentedORBDetect(img, orbInstance, nBlocks=(2,2)):        # TODO Under update
    numRows, numCols = nBlocks
    horizontal = np.array_split(img, numRows)
    splitted_img = [np.array_split(block, numCols, axis=1) for block in horizontal]
    ...
    for row in range(numRows):
        for col in range(numCols):
            ...
            keypoints = orbInstance.detect(currentImg, None)
            numFeatures = orbInstance.getMaxFeatures()

            for kp in keypoints:
                kp.pt = (kp.pt[0]+widthOffset, kp.pt[1]+heightOffset)
                result.append(kp)

    #assert len(result) == numFeatures, f"Function segmentedORBDetect: Missmatch between numFeatures and result size.\n\tExpected: {len(result)}\n\tDetected: {numFeatures}"
    return result
...
def computeMatchesScore(final_matches, imgKeypoints, subKeypoints, resizeScale, x0, y0, cx, cy, rotAngle, thresholdRadius, matcherType, pitch, roll, numBestMatches, bestMatchesMultiplier=3):
    ...
    for i in range(len(final_matches)):
        ...
        if matcherType == cv2.NORM_HAMMING:
            ...
            print(f"Cost function for NORM_HAMMING2 distance was not developed yet.")
            exit()
        inliers += bValidPoint
